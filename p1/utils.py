...
class OwnPriorityQueue(Queue):
    ...
    def append(self, item):
        ...
        self.array_ = sorted(self.array_, key=lambda x: x.path_cost)

    def extend(self, items):
        self.array_.extend(items)
        self.array_ = sorted(self.array_, key=lambda x: x.path_cost)
    ...
    def pop(self):
        return self.array_.pop(0) if self.array_ else None


class OwnPriorityQueueWithH(Queue):
    
    def __init__(self):
        self.array_ = []
        self.lambda_sort = lambda x: x.path_cost + x.heuristic_value

    def append(self, item):
        self.array_.append(item)
        self.array_ = sorted(self.array_, key=self.lambda_sort)
