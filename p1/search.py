...
class Node:
    ...
    def __init__(self, state, parent=None, action=None, path_cost=0, heuristic_func=None):
        """Create a search tree Node, derived from a parent by an action."""
        update(self, state=state, parent=parent, action=action,
               path_cost=path_cost, depth=0)
        self.heuristic_value = heuristic_func(self) if heuristic_func else 0
        if parent:
    ...
    def heuristic_value(self):
        return self.heuristic_func(self)
...
def priority_choosing_search(problem):
    return graph_search(problem, OwnPriorityQueue())

def priority_choosing_with_h_search(problem):
    return graph_search(problem, OwnPriorityQueueWithH())
